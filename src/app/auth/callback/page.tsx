...
export default function AuthCallback() {
  ...
  useEffect(() => {
    const handleAuth = async () => {
      try {
        // Supabase browser client automatically catches the hash tokens
        // and sets the session in memory/localStorage.
        const { data: { session }, error: sessionError } = await supabase.auth.getSession();
        
        if (sessionError) throw sessionError;
        
        if (session) {
          // Success! Whisk them away
          router.push("/dashboard/youtube");
        } else {
          // If no session after a few seconds, something went wrong
    ...
    // Listen for auth state changes (in case the hash takes a second to process)
